...
public class DirectedCallGraph implements DirectedGraph {
    ...
    private static Hashtable<SootMethod,Hashtable<SootMethod,Double>> localFreq = new Hashtable<SootMethod,Hashtable<SootMethod,Double>>();
    private static Hashtable<SootMethod,Hashtable<SootMethod,Double>> backEdgeProb = new Hashtable<SootMethod,Hashtable<SootMethod,Double>>();
    
    public void setLocalFreq(SootMethod from, SootMethod to, Double what) {
    	if(!localFreq.containsKey(from)) {
    		localFreq.put(from, new Hashtable<SootMethod,Double>());
    	}
    	
    	localFreq.get(from).put(to, what);
    }
    
    public double getLocalFreq(SootMethod from, SootMethod to) {
    	if(localFreq.containsKey(from) && localFreq.get(from).containsKey(to)){
    		return localFreq.get(from).get(to);
    	}else {
    		return 0.0;
    	}
    }
    
    public void setBackEdgeProb(SootMethod from, SootMethod to, Double what) {
    	if(!backEdgeProb.containsKey(from)) {
    		backEdgeProb.put(from, new Hashtable<SootMethod,Double>());
    	}
    	
    	backEdgeProb.get(from).put(to, what);
    }
    
    public double getBackEdgeProb(SootMethod from, SootMethod to) {
    	if(backEdgeProb.containsKey(from) && backEdgeProb.get(from).containsKey(to)){
    		return backEdgeProb.get(from).get(to);
    	}else {
    		return 0.0;
    	}
    }
    ...
    private Hashtable<SootMethod,Double> invokeFreq = new Hashtable<SootMethod,Double>();
    private Hashtable<SootMethod,Hashtable<SootMethod,Double>> globalCallFreq = new Hashtable<SootMethod,Hashtable<SootMethod,Double>>();
    
    public void setGlobalCallFreq(SootMethod from, SootMethod to, Double what) {
    	if(!globalCallFreq.containsKey(from)) {
    		globalCallFreq.put(from, new Hashtable<SootMethod,Double>());
    	}
    	
    	globalCallFreq.get(from).put(to, what);
    }
    
    public double getGlobalCallFreq(SootMethod from, SootMethod to) {
    	if(globalCallFreq.containsKey(from) && globalCallFreq.get(from).containsKey(to)){
    		return globalCallFreq.get(from).get(to);
    	}else {
    		return 0.0;
    	}
    }
    
    public void setInvokeFreq(SootMethod who, Double what) {
    	invokeFreq.put(who, what);
    }
    
    public double getInvokeFreq(SootMethod who) {
    	if(invokeFreq.containsKey(who)){
    		return invokeFreq.get(who);
    	}else {
    		return 0.0;
    	}
    }
