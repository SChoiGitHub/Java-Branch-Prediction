...
public class Block implements Iterable<Unit>
{
    ...
    private Hashtable<Block,Double> edgeProbability = new Hashtable<Block,Double>();
    private Hashtable<Block,Double> backEdgeProbability = new Hashtable<Block,Double>();
    
    private Hashtable<Block,Double> edgeFrequency = new Hashtable<Block,Double>();
    
    
    
    
    
    private ArrayList<SootMethod> whoDoICall = new ArrayList<SootMethod>();
    private double blockFrequency = 0.0;
    
    public ArrayList<SootMethod> whoDoICall() {
    	return whoDoICall;
    }
    public void calls(SootMethod sm) {
    	whoDoICall.add(sm);
    }
    
    		
    private Boolean _visited = true;
    public Boolean visited() { return _visited; }
    ...
    public void setEdgeProb(Block b, double d) {
    	edgeProbability.put(b,d);
    }
    public double getEdgeProb(Block to) {
    	return edgeProbability.get(to);
    }
    
    
    
    public void setEdgeFreq(Block b, double d) {
    	edgeFrequency.put(b,d);
    }
    public double getEdgeFreq(Block to) {
    	if(edgeFrequency.containsKey(to)) {
    		return edgeFrequency.get(to);
    	}else {
    		//System.out.println(getHead() + "\t" + to.getHead());
    		//I found (what is at least a general occurrence) that sometimes edges do not exist because they are exceptions?
    		return 0.0;
    	}
    }
    
    public void intializeBackEdgeProb() {
    	for(Block b : edgeFrequency.keySet()) {
    		backEdgeProbability.put(b, edgeFrequency.get(b));
    	}
    }
    public void setBackEdgeProb(Block b, double d) {
    	backEdgeProbability.put(b,d);
    }
    
    public double getBackEdgeProb(Block to) {
    	return backEdgeProbability.get(to);
    }
    
    
    public void setBlockFreq(double d) {
    	blockFrequency = d;
    }
    public double getBlockFreq() {
