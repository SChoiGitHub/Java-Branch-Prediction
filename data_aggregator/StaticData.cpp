...
StaticData::StaticData(){
	throw(std::runtime_error("Error: Missing file to parse"));
}
StaticData::~StaticData(){
	...
	if(inFile.is_open()){
		while(getline(inFile,line)){
			...
			if(line.substr(0,6) == "[ToIB]"){ //We know it begins here.
				...
				method_name_to_taken.insert({line.substr(9),std::vector<int*>()}); //get the method name
				getline(inFile,line); //get next line, that will be the number of lines we deal with;
				parse_method(line.substr(9), std::stoi(line.substr(21)));
			}
		}
	}else{
		throw(std::runtime_error("Error: Missing file to parse"));
...
void StaticData::parse_method(std::string method_name, int lines){
	...
	for(int x = 0; x < lines; x++){
		...
		method_name_to_taken[method_name].push_back(h_data_for_this_line);
