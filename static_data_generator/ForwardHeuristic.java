...
public class ForwardHeuristic extends HeuristicBase {
	
	ForwardHeuristic(HeuristicDatabase hd, int heuristic_id){
	...
	protected void internalTransform(Body b, String phaseName, Map options){
		...
		for(Unit u1 : units){
			...
			try{
				...
				if(!units.follows(u1,(Unit)ifStatement.getTarget())){
					hd.add(b.getMethod(),h_id,false,ifStatement);
				}
			}catch(Exception e){
				//Ignore this...
				continue;
